...
/** Upsert default clauses by name so existing installs pick up new/updated language. */
async function seedContractTemplates() {
  let inserted = 0;
  let updated = 0;
  for (const template of DEFAULT_TEMPLATES) {
    const existing = await ContractTemplate.findOne({ name: template.name }).select('_id').lean();
    await ContractTemplate.updateOne(
      { name: template.name },
      {
        ...
        $setOnInsert: { name: template.name }
      },
      { upsert: true }
    );
  ...
  if (inserted > 0 || updated > 0) {
    console.log(`✅ Contract templates: ${inserted} inserted, ${updated} updated`);
  }
}
