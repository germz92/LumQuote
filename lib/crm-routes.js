...
module.exports = function registerCrmRoutes(app, ctx) {
  ...
  app.post('/api/projects', requireApiAuth, async (req, res) => {
    try {
      const { name, status, startDate, endDate, notes, leadSource } = req.body;
      if (!name || !name.trim()) {
    ...
    try {
      ...
      const [quotes, contract, invoices] = await Promise.all([
        SavedQuote.find({ project: project._id }, {
          name: 1, clientName: 1, location: 1, leadSource: 1, booked: 1, archived: 1,
  ...
  app.put('/api/projects/:id', requireApiAuth, async (req, res) => {
    try {
      ...
      const { name, status, startDate, endDate, notes, clientId, leadSource } = req.body;
      const update = {};
      if (name !== undefined) update.name = name.trim();
...
(function () {
  ...
  app.get('/api/invoices', requireApiAuth, async (req, res) => {
    try {
      ...
      // Unpaid/overdue already define the set; don't also apply upcoming/past.
      if (status !== 'unpaid' && status !== 'overdue') {
        if (when === 'past') {
          and.push({
            dueDate: { $exists: true, $nin: [null, ''], $lt: today }
