...
async function notifyStaffContractSigned(contract, { baseUrl }) {
  const proj = await loadProjectForNotify(contract.project);
  const to = await resolveStaffNotifyEmails(proj);
  if (to.length === 0) return { ok: false, skipped: true, to: [], error: 'No staff recipients' };
  if (hasEmailLog(contract, 'owner_signed')) return { ok: true, skipped: true, to };

  const signer = contract.signature?.name || 'Client';
  const link = `${baseUrl}/projects/${contract.project}`;
  const html = wrapEmailHtml({
...
async function notifyStaffInvoicePaid(invoice, { baseUrl, installmentIndex = null }) {
  const proj = await loadProjectForNotify(invoice.project);
  const to = await resolveStaffNotifyEmails(proj);
  if (to.length === 0) return { ok: false, skipped: true, to: [], error: 'No staff recipients' };
  ...
  if (isFull && hasEmailLog(invoice, 'owner_paid')) return { ok: true, skipped: true, to };
  if (!isFull && hasEmailLog(invoice, 'owner_paid_installment', { installmentIndex })) {
  ...
  const link = `${baseUrl}/projects/${invoice.project}?invoice=${invoice._id}`;
  const html = wrapEmailHtml({
