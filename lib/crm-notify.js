...
async function emailContractAndInvoice({ contract, invoice, email, baseUrl, project }) {
  ...
  if (!includeContract && !includeInvoice) {
    return { ok: true, skipped: true, to: [], error: 'nothing_to_send' };
  }
  ...
  const company = (await getCompanySettings()).companyName || 'Lumetry Media';
  const projectName = proj?.name || contract?.title || 'your project';
  const hi = proj?.client?.name ? ` ${proj.client.name}` : (invoice?.to?.name ? ` ${invoice.to.name}` : '');
  ...
  if (includeContract && includeInvoice) {
    ...
    bodyHtml = `<p>Hi${hi},</p>
      <p>${company} has sent your contract and invoice for <strong>${projectName}</strong>.</p>
    ...
    bodyHtml = `<p>Hi${hi},</p>
      <p>${company} has sent you a contract for <strong>${projectName}</strong>.</p>
      <p>Open the link below to review and sign.</p>`;
  } else {
